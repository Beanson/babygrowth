            ...
            public boolean onTouch(View v, MotionEvent event) {
                ...
                switch (event.getAction()){
                    case MotionEvent.ACTION_DOWN:{
                        //Log.i(DEBUG_TAG, "on down  x=" +event.getX()+"  ;y="+event.getY() );
                        MyResource.mark=true;
                        MyResource.start_x= (int) event.getX();
                        MyResource.start_y= (int) event.getY();
                    }
                    case MotionEvent.ACTION_MOVE:{
                        //Log.i(DEBUG_TAG, "on move  x=" +event.getX()+"  ;y="+event.getY() );
                        MyResource.current_x= (int) event.getX();
                        MyResource.current_y= (int) event.getY();
                        MyResource.mark_pre=false;
                    }
                    case MotionEvent.ACTION_UP:{
                        //Log.i(DEBUG_TAG, "on OUTp  x=" +event.getX()+"  ;y="+event.getY() );
                        if(MyResource.mark_pre){
                            MyResource.mark=false;
                        }
                        MyResource.mark_pre=true;
                    }
                }
                return true;
            }
